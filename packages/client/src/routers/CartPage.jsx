...
const CartPage = ({ mykurlyService }) => {
  ...
  const onChangeProductCount = (cart, count) => {
    const cart_update = {
      ...
      total_accumulate_price: cart.total_accumulate_price,
      payment_price: cart.payment_price,
      cart_detail_seq: cart.cart_detail_seq,
      product_view_seq: cart.product_view_seq,
      product_seq: cart.product_seq,
                ...
                <div className={styles.category}>
                  ...
                  <ul style={{ padding: '0' }}>
                    {Object.keys(cartList).map(
                      (key) =>
                        cartList[key].packaging_type === '냉장 (종이포장)' && (
                          <li key={key}>
