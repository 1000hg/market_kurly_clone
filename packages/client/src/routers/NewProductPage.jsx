...
export default function NewProductPage({ mykurlyService }) {
  ...
  const paramHandler = () => {
    ...
    if (query.brand === null || query.brand === undefined) {
      if (query.category_seq === null || query.category_seq === undefined) {
        params = {
          page: query.page,
  ...
  const makeUrl = (pageNum, sortIdx) => {
    debugger;
    //카테고리
    let categorySeq = query.category_seq;
    //브랜드
    let brand = query.brand;
    //정렬
    let sortType = sortIdx !== "" ? sortIdx : query.sort_type;
    //새로운 url
    let newUrl = location.pathname + "?page=" + pageNum;
  ...
  const onClickSortType = (idx) => {
    ...
    if (query.sort_type === null || query.sort_type === undefined) {
      navigate(makeUrl(1, 0));
    } else {
      navigate(makeUrl(1, idx));
      axios
  ...
  useEffect(() => {
    //  /collections/market-newproduct 경로를 쳐서 들어가는 경우 처리
    if (query.page === null) {
      query.page = 1;
    }
    if (query.sort_type === undefined) {
      query.sort_type = 0;
    }
  ...
  return (
    <>
      ...
      <div>
        <p className={styles.sbTitle}>신상품</p>
      </div>
